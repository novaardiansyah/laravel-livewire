  ...
  <div class="card-body">
    ...
    <form action="" method="post" enctype="multipart/form-data" wire:submit="createNewUser">
      ...
      <div class="mb-3">
        ...
        <input type="file" class="form-control" name="image" wire:model="image" accept="image/png, image/jpg, image/jpeg" wire:model="image" id="{{ rand() }}" />
        @error('image')
          <div class="invalid-feedback d-block">{{ $message }}</div>
        @enderror
      ...
      <div class="text-end" wire:loading.remove>
        ...
        <button type="button" class="btn btn-sm btn-success" @click="$dispatch('createNewUser')">Reload List</button>
        <button type="button" class="btn btn-sm btn-danger" wire:click="deleteAllUser">Delete All User</button>
      </div>
    </form>
  </div>
</div>